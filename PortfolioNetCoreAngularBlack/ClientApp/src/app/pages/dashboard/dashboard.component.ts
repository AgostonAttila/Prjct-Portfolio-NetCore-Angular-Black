...
export class DashboardComponent implements OnInit {
  ...
  labelLittleCharts = ['JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];

  labelSums = ['YTD', '6 MONTH', '1 YEAR', '3 YEAR', '5 YEAR'];
  ...
  public updateRedChart() {
    ...
    this.myChartDataRed.options.scales.yAxes[0].ticks.suggestedMin = Math.min.apply(Math, this.dataPerfChart) * 0.9;
    this.myChartDataRed.options.scales.yAxes[0].ticks.suggestedMax = Math.max.apply(Math, this.dataPerfChart) * 1.1;
    this.myChartDataRed.data.labels = this.labelLittleCharts;
    this.myChartDataRed.update();
  }

  public updateGreenChart() {
    this.myChartDataGreen.data.datasets[0].data = this.dataSharpeChart;
    this.myChartDataGreen.options.scales.yAxes[0].ticks.suggestedMin = Math.min.apply(Math, this.dataSharpeChart) * 0.9;
    this.myChartDataGreen.options.scales.yAxes[0].ticks.suggestedMax = Math.max.apply(Math, this.dataSharpeChart) * 1.1;
    this.myChartDataGreen.update();
  }

  public updateBlueChart() {
    this.myChartDataBlue.data.datasets[0].data = this.dataVolatilityChart;
    this.myChartDataBlue.options.scales.yAxes[0].ticks.suggestedMin = Math.min.apply(Math, this.dataVolatilityChart)*0.9;
    this.myChartDataBlue.options.scales.yAxes[0].ticks.suggestedMax = Math.max.apply(Math, this.dataVolatilityChart)*1.1;
  ...
  public changeSelectedFund(fund: Fund) {
    ...
    this.labelLittleCharts = [];

    var today = new Date();

    for (var i = 0; i < 6; i++) {
      var date = new Date(today.getFullYear(), today.getMonth() - i, today.getDay());
      this.labelLittleCharts = [this.mainChartLabels[date.getMonth()]].concat(this.labelLittleCharts);
    }
    ...
    for (var i = 0; i < 6; i++) {
      ...
      if (date.getFullYear() === today.getFullYear()) {
        this.dataPerfChart.push(fund.monthlyPerformanceList[fund.monthlyPerformanceList.length - 1].performanceListByMonth[date.getMonth()]);
      }
      else {
        this.dataPerfChart.push(fund.monthlyPerformanceList[fund.monthlyPerformanceList.length - 2].performanceListByMonth[date.getMonth()]);
      }
    }

    this.dataPerfChart = this.dataPerfChart.reverse();

    this.performanceSum = Math.round(this.dataPerfChart.reduce(reducer) * 100) / 100;
    this.updateRedChart();

    //VOLATILITY BLUE     
    this.dataVolatilityChart = fund.volatilityArray.map(x => { if (x !== '-') { return parseFloat(x.replace(",", ".")) } else { return 0 } });
    this.volatilitySum = Math.round(this.dataVolatilityChart.reduce(reducer) * 100) / 100;
    this.updateBlueChart();

    //SHARPE GREEN
    this.dataSharpeChart = fund.sharpRateArray.map(x => { if (x !== '-') { return parseFloat(x.replace(",", "."))  } else { return 0 } });
